<#
ax_detached.ps1 — FLASH-FREE agy dispatch (no-admin runtime trigger).
...
ax_detached_setup (see header of that command). Thereafter THIS script only:
  1. writes the per-run request to .ax_detached_request.json
  2. triggers the pre-registered task 'RIMA_ax_detached' (running your OWN task = no admin)
...
session -> use ax_dispatch.cmd for those (accept the minor flash).

USAGE (Claude PowerShell tool, run_in_background recommended):
  & ".\ax_detached.ps1" `
      -TaskFile "tasks/foo.md" [-Account <prefix>] [-PrintTimeout 600]
#>
param(
...
$root     = $PSScriptRoot
$taskName = 'ax_detached'

# Bail early with a clear message if the one-time admin setup hasn't been run.
$task = Get-ScheduledTask -TaskName $taskName -ErrorAction SilentlyContinue
if (-not $task) {
    Write-Error "Task '$taskName' not registered. Run the one-time admin setup first (see ax_detached_setup command)."
    exit 3
}
...
$req | ConvertTo-Json | Set-Content -Path (Join-Path $root '.ax_detached_request.json') -Encoding UTF8
...
if ($after -and $after.LastWriteTime -gt $beforeStamp) {
    Write-Output "[ax_detached] OK — $($after.Name) (modified $($after.LastWriteTime)):"
    Get-Content $after.FullName -Raw
} else {
    Write-Error "[ax_detached] no fresh AGY_DONE produced (state=$state). ConPTY may not init in the S4U session — fall back to hidden-desktop approach."
    exit 1
}
